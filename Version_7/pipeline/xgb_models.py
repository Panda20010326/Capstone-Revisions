...
class XGBModels:
    ...
    def _encode(self, profile: dict) -> pd.DataFrame:
        # Use object dtype so encoded integers can safely replace categorical strings.
        row = pd.DataFrame(
        ...
        # Collapse rare categories using the same mapping used during training.
        for col, rare_values in self.rare_map.items():
            if col not in row.columns:
                continue
            value = row.at[0, col]
            if pd.notna(value) and value in rare_values:
                row.at[0, col] = "Other"
        ...
        # Ensure encoded categorical columns are numeric.
        for col in self.encoders:
            if col in row.columns:
                row[col] = pd.to_numeric(row[col], errors="raise").astype("int64")

        # Ensure the remaining numeric features are numeric too.
        for col in FEATURES:
