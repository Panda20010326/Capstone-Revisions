...
# Conservative land envelopes for the cities used by the notebook. These are
# intentionally inland enough to remove the synthetic/offshore CMHC points that
# otherwise appear in Lake Ontario on the Folium map.
CITY_LAND_BOUNDS = {
    "toronto": (43.590, 43.82, -79.65, -79.12),
    "mississauga": (43.50, 43.75, -79.82, -79.42),
    "burlington": (43.30, 43.50, -80.10, -79.65),
    "hamilton": (43.18, 43.38, -80.15, -79.65),
...
def prepare_housing_data(housing_df: pd.DataFrame) -> pd.DataFrame:
    """Normalize housing coordinates and remove obviously invalid/offshore points."""
    if housing_df.empty:
    ...
    housing = housing[housing.apply(lambda r: _valid_coordinates(r["lat"], r["lon"]), axis=1)]
    housing = housing[housing.apply(lambda r: _looks_like_land(r.get("city", ""), float(r["lat"]), float(r["lon"])), axis=1)]
...
def filter_housing_near_jobs(
    ...
    homes = prepare_housing_data(housing_df)
    if homes.empty:
        return homes
...
def rank_housing_for_job(housing_df: pd.DataFrame, job: pd.Series, profile: dict[str, Any]) -> pd.DataFrame:
    ...
    housing = prepare_housing_data(housing_df)
