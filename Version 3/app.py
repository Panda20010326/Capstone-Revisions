...
def cached_get_jobs(keyword: str, city: str):
...
with st.status("Running pipeline...", expanded=True) as status:
    ...
    status.update(label="Stage 4/5 — Adzuna API")
    search_keyword = build_search_keyword(
        profile_result.predicted_occupation, previous_occupation, keyword_override
    )
    jobs_df = cached_get_jobs(search_keyword, preferred_city)
    st.write(f"Search: `{search_keyword}` in `{preferred_city}` → **{len(jobs_df)}** jobs retrieved.")
