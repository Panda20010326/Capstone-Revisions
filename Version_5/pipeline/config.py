...
# Offline alternative to the live API: a dataset already pulled from Adzuna
# and saved to CSV (Employment_Prediction/Datasets/processed_adzuna_jobs.csv
# in the original project -- collected via adzuna_job_analysis.py for the
# single search "data analyst" in Toronto). Same output columns as the live
# API wrapper, so it's a drop-in swap. See pipeline/job_source.py.
LOCAL_JOBS_DATASET_PATH = os.path.join(DATA_DIR, "processed_adzuna_jobs.csv")
JOB_SOURCE_DEFAULT = "local"  # "local" or "live"
