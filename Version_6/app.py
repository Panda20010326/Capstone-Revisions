...
This version keeps training outside the app. It loads corrected XGBoost inference
artifacts and only uses optional ProfileEncoder artifacts when a complete inference
...
if page == "How This Works":
    ...
    st.info(
        "Prediction models are loaded from saved `.pkl` artifacts; the app does not retrain "
        "models on every click. The current XGBoost artifacts were retrained with "
        "`occupation_category` so every runtime feature is available from the form and "
        "the outcome-derived `employment_category` field is not used."
    )
    ...
    with st.status("Finding your matches...", expanded=True) as status:
        try:
            ...
            if profile_result.used_fallback:
                st.warning(
                    "The full ProfileEncoder multitask inference artifacts are not included in this package, "
                    "so this run used the profile-category fallback for predicted occupation and "
                    "fit score. See **How This Works** in the sidebar for details."
                )
            st.write(
