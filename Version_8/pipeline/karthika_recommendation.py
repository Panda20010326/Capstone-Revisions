...
def load_housing_data(path: str | Path | None = None) -> pd.DataFrame | None:
    """Load the geocoded housing dataset from disk.
 
    If ``path`` is not supplied, falls back to ``config.HOUSING_DATA_PATH``
    (imported lazily here to avoid a circular import with the pipeline
    package). Returns ``None`` if the file cannot be found, matching the
    behaviour app.py previously relied on.
    """
    if path is None:
        from . import config  # local import avoids circular import at module load time
 
        path = config.HOUSING_DATA_PATH
