...
from pipeline.job_source import get_jobs_from_dataset, dataset_info, dataset_cities
...
if submitted:
    with st.status("Finding your matches...", expanded=True) as status:
        try:
            ...
            st.write(f"Calibrated employment probability: **{ei_result.employment_probability:.0%}**")
            ...
            if use_local_jobs:
                ...
                # The bundled local job file is Toronto-heavy. Never silently
                # substitute Toronto jobs for another requested city. If live
                # Adzuna credentials are configured, automatically use them for
                # cities not represented in the local file.
                if jobs_df.empty and config.ADZUNA_APP_ID and config.ADZUNA_APP_KEY:
                    jobs_df = cached_get_jobs_live(search_keyword, preferred_city)
                    source_label = "live Adzuna API (automatic city fallback)"
            ...
            if use_local_jobs and not jobs_df.empty:
                st.caption(
                    "ℹ️ Local jobs are used only when the requested city is represented in "
                    "the bundled dataset. The app will not substitute Toronto jobs for another city."
                )
            elif use_local_jobs and jobs_df.empty:
                if config.ADZUNA_APP_ID and config.ADZUNA_APP_KEY:
                    st.warning(
                        f"No local job postings are available for **{preferred_city}**. "
                        "The app attempted a live Adzuna search for that city but returned no jobs."
                    )
                else:
                    st.warning(
                        f"No local job postings are available for **{preferred_city}**. "
                        "The bundled job dataset is Toronto-heavy. Add ADZUNA_APP_ID and "
                        "ADZUNA_APP_KEY or select **Live Adzuna API** to get real jobs for this city."
                    )
            ...
            if jobs_df.empty:
                ...
                # Karthika's notebook converted to a runtime Python module.
                karthika_profile = {
                    ...
                    "years_of_experience": years_of_experience,
                    "TEER_category": teer_category,
                    "regulated_profession": regulated_profession == "Yes",
                    "credential_recognition_status": credential_recognition_status,
                    ...
                    "homes_per_job": 5,
                    "employment_probability": ei_result.employment_probability,
                    "predicted_income": ei_result.predicted_income or 65000,
                    "profile_fit_score": profile_result.profile_fit_score,
                }

                ranked_jobs = karthika_rank_jobs(jobs_df, karthika_profile).head(10).copy()
                housing_recommendations = karthika_build_housing_recommendations(
                    housing_df if housing_df is not None else pd.DataFrame(),
                    ranked_jobs,
                    karthika_profile,
                )
...
housing_recommendations = results.get("housing_recommendations", pd.DataFrame())
...
support2.metric(
    ...
    help="A calibrated estimate of the likelihood of employment within the first year. "
    "Calibration makes the percentage better reflect the observed employment rate in "
    "the training data instead of displaying an overconfident raw XGBoost score.",
)
st.caption(
    "Probability is calibrated against held-out training data; it is an estimate, not a guarantee of employment."
)
...
# ---------------------------------------------------------------------------
# Folium interactive map — Karthika recommendations only
# ---------------------------------------------------------------------------
...
if not map_jobs.empty:
    map_jobs["latitude"] = pd.to_numeric(map_jobs.get("latitude"), errors="coerce")
    map_jobs["longitude"] = pd.to_numeric(map_jobs.get("longitude"), errors="coerce")
    map_jobs = map_jobs.dropna(subset=["latitude", "longitude"])
    map_jobs = map_jobs[
        map_jobs["latitude"].between(40, 60)
        & map_jobs["longitude"].between(-95, -70)
    ]

if not map_homes.empty:
    # Re-apply the same Karthika housing validation used during ranking.
    # This is important because the map should never display an unfiltered
    # housing coordinate, even if an upstream dataframe changes.
    map_homes = prepare_housing_data(map_homes, preferred_city)
    map_homes["lat"] = pd.to_numeric(map_homes["lat"], errors="coerce")
    map_homes["lon"] = pd.to_numeric(map_homes["lon"], errors="coerce")
    map_homes = map_homes.dropna(subset=["lat", "lon"])

if map_jobs.empty and map_homes.empty:
    st.info("No valid recommended job or nearby housing coordinates were available for the map.")
else:
    coordinate_pairs = []
    if not map_jobs.empty:
        coordinate_pairs.extend(map_jobs[["latitude", "longitude"]].astype(float).values.tolist())
    if not map_homes.empty:
        coordinate_pairs.extend(map_homes[["lat", "lon"]].astype(float).values.tolist())

    center_lat = sum(p[0] for p in coordinate_pairs) / len(coordinate_pairs)
    center_lon = sum(p[1] for p in coordinate_pairs) / len(coordinate_pairs)
    ...
    jobs_layer = folium.FeatureGroup(name="Recommended jobs", show=True)
    homes_layer = folium.FeatureGroup(name="Nearby housing", show=True)

    for _, job in map_jobs.iterrows():
        score = float(job.get("karthika_job_score", job.get("match_score", 0)) or 0)
        marker_color = "green" if score >= 70 else "orange" if score >= 40 else "red"
        ...
        popup = (
            ...
            f"Karthika match score: {score:.1f}%"
        )
        folium.Marker(
            location=[float(job["latitude"]), float(job["longitude"])],
            popup=folium.Popup(popup, max_width=300),
            tooltip=f"Job: {title} ({score:.0f}%)",
            icon=folium.Icon(color=marker_color, icon="briefcase", prefix="fa"),
        ).add_to(jobs_layer)

    for _, home in map_homes.iterrows():
        rent = float(home.get("monthly_rent", 0) or 0)
        commute = float(home.get("commute_km", 0) or 0)
        affordable = bool(home.get("affordable", False))
        score = float(home.get("combined_score", 0) or 0)
        neighbourhood = str(home.get("neighbourhood", "Housing"))
        city = str(home.get("city", ""))
        job_title = str(home.get("job_title", ""))
        popup = (
            f"<b>{neighbourhood}, {city}</b><br>"
            f"Rent: ${rent:,.0f}/month<br>"
            f"Near: {job_title}<br>"
            f"Commute: {commute:.1f} km<br>"
            f"Affordable: {'Yes' if affordable else 'No'}<br>"
            f"Combined score: {score:.1f}"
        )
        folium.Marker(
            location=[float(home["lat"]), float(home["lon"])],
            popup=folium.Popup(popup, max_width=300),
            tooltip=f"Housing: {neighbourhood} — {commute:.1f} km from job",
            icon=folium.Icon(
                color="blue" if affordable else "orange",
                icon="home",
                prefix="fa",
            ),
        ).add_to(homes_layer)
    ...
    job_map.fit_bounds(coordinate_pairs, padding=(25, 25))
    st_folium(job_map, use_container_width=True, height=550)

    st.caption(
        "🟢 Strong job match (≥70%)   🟠 Moderate (40–70%)   🔴 Weaker (<40%)   "
        "🔵 Nearby affordable housing   🟠 Housing above the affordability threshold"
    )
